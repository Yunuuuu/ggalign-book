...
`heatmap_layout()` is a specialized version of `quad_alignb()`, which is a
special `QuadLayout` (`quad_layout()`) that aligns observations both
horizontally and vertically. We introduce `heatmap_layout()` directly, as it is
more familiar to many users, especially those experienced with popular heatmap
packages like `pheatmap` and `ComplexHeatmap`.
...
## input data
As mentioned in @sec-stack_layout_input_data, we typically require a matrix for
...
## heatmap body
A `ggplot` object will be automatically created for the heatmap body, the matrix
input will be converted into a long formated data frame when drawing. The data
in the underlying `ggplot` object contains following columns:
...
By default, the heatmap body is regarded as the active plot, meaning you can add
ggplot2 elements directly to the heatmap body. 
```{r}
...
automatically create a new `stack_layout()` (either `stack_align()` or
`stack_free()`) depending on whether you want to align observations in the
specified direction. For top and bottom annotations, `stack_alignv()` or
`stack_freev()` will be used; for left and right annotations, `stack_alignh()`
or `stack_freeh()` will be applied.
...
described in @sec-stack_layout), since the annotation is a `Layout` object.
...
By default, the annotation `stack_layout()` will inherit data from
...
stack, as described in @sec-stack_layout.
```{r}
ggheatmap(small_mat) +
...
`quad_alignv()`, which only align observations in one direction, the data in the
layout may not fit the data for the annotation (when the layout requires
alignment of observations, we typically use a matrix, regardless of whether
alignment is needed in one or two directions)
...
In both conditions, `quad_anno()` won't initialize the annotation by default,
instead, you must provide the annotation `stack_layout()` manually.

## Adding stack layout
Like adding plot in `stack_layout()` (@sec-stack_layout), when the direction
neeed alignment, you can add a `stack_layout()` regardless of whether it need
to align observations.
