...
`heatmap_layout()` is a specialized version of `quad_discrete()`, which itself
is a specific variant of `QuadLayout` (`quad_layout()`) designed to align
discrete variables both horizontally and vertically. We introduce
...
## input data
As mentioned in @sec-align-variable, we typically require a matrix for the
