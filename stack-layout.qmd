...
ggplot object of `align_dendro()` consists of the dendrogram node data, which
can be used to further customize the dendrogram visualization. However, it is
possible to use the dendrogram's edge data for customization, which I will
...
Both function will by default set the active plot when added to the layout.
...
arranges plots in a single row or column without enforcing axis alignment:
```{r}
stack_freev(mpg) +
