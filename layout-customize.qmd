...
As mentioned in @sec-heatmap_layout_annotations, for top and bottom annotations,
...
based on hierarchical clustering. Unlike `align_dendro()`, however,
`align_hclust()` does not add a dendrogram tree to the plot. All the arguments
intruduced here can also be used by `align_dendro()`.
...
observations within a heatmap layout. Now, having set the stage for proper
observation alignment, we can move on to the next essential step: `plot
initialize`. In the following chapter, we'll dive deeper into how to set up your
