# Layout customization {#sec-layout-customization}

For layouts that can align observations (discrete variables), the package
provides a suite of `align_*` functions designed to give you precise control
over the observations. These functions allow you to reorder observations or
partition them into multiple groups.
...
the mean value across all columns in the heatmap body matrix.
...
>Developers can extend this functionality by implementing a custom `order2()`
method for their object classes.
...
alignment, we can move on to the next essential step: `plot initialization`. In
the following chapter, we'll dive deeper into how to set up your plots within
the layout.
