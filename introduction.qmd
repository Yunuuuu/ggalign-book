...
graph TD
    %% Input objects
    subgraph "Input Objects"
        G["ggplot"]
        H["ComplexHeatmap, pheatmap, base plot with formula, et al."]
    end

    %% patch() as optional wrapper
    PatchFn["patch() - (optional) wraps input into Patchable"]

    Patch["**Patch object system**<br/>The detailed implementation of layout, alignment, and compositional logic"]

    %% align_plots as the core engine
    Align["align_plots()<br/>Core layout engine"]
    ...
    subgraph Modifiers
         ...
         F2["free_space()"]
         F2Desc["Remove spacing for compact layout"]
         F3["free_border()"]
         F3Desc["Attach border elements directly to panels"]
         F4["free_lab()"]
         F4Desc["Attach axis label/title elements directly to panels"]
         F5["free_vp()"]
         F5Desc["Customize viewport placement"]
         F6["free_guide()"]
         F6Desc["Manually override guide (legend) alignment"]
    end
