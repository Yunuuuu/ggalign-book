...
```{mermaid}
flowchart TD
    subgraph InputObjects["Input plot (Plot Objects Ocean)"]
        G["ggplot"]
    ...
    H --> PatchFn & Patch
    G --> Patch
    InputObjects --> Align["**align_plots()**<br>Core data-free composition engine<br>"] & Modifiers
    Modifiers --> Align & Patch
    PatchFn --> Patch
    F1 --> F1Desc
    F2 --> F2Desc
    F3 --> F3Desc
    ...
    style InputObjects fill:none,stroke:#888888,stroke-width:2px,font-size:12px
    style Align fill:#BBDEFB,stroke:#2196F3,stroke-width:2px,color:#0D47A1,font-size:12px
    style Modifiers fill:#FFF3E0,stroke:#FFB300,stroke-width:2px,color:#E65100,font-size:12px
    style Render fill:#C5CAE9,stroke:#3F51B5,stroke-width:2px,color:#1A237E,font-size:12px
    style Internal fill:#FFF9C4,stroke:#FBC02D,stroke-width:2px,color:#827717,font-size:12px
```
