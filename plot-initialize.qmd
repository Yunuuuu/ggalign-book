...
axis titles, you must do so manually by customizing the theme.
...
## `ggmark()`
It would be helpful if we can add an annotation plot for the selected observations, 
`ggmark()` can be used to do this. 
...
By default, `ggmark()` will connect all observations and split them based on the
groups defined in the layout. The data underlying the ggplot object created by
`ggmark()` is similar to that of `ggalign()` (@sec-ggalign), but it does not
include the `.x`/`.y` and `.discrete_x`/`.discrete_y` columns.

You can use the `plot.margin` argument to control the link size.
...
The `mark_draw()` function in the package allows you to define linked
observations for each panel using a list of `link()` objects, which 
accepts two arguments:

  - `link1`: An integer or character index that specifies the linked observations.
  - `link2`: Used to link observations in the right or bottom, this should match
  `link1`, as all plots in stack_discrete() should maintain the same ordering
  index. If you want `link2` to inherit from `link1`, you can use the `waiver()`
  directly (the same for `link1` argument, you can use `waiver()` to inherit
  from `link2` argument). However, this becomes useful in `stack_cross()`, where
  different orderings are involved, and you must specify the links explicitly.
...
ggheatmap(small_mat) +
    ...
    anno_right() +
    align_kmeans(3L) +
    ggmark(mark_line(link(4:6), link(1:2))) +
...
ggheatmap(small_mat) +
    ...
    ggmark(mark_line(link(4:6), link(1:2))) +
    geom_boxplot(aes(.names, value, fill = .names)) +
    theme(plot.margin = margin(l = 0.1, r = 0.1, t = 0.1, unit = "npc"))
```
::: 

:::

You can also use `waiver()` to inherit links from the other argument:
```{r}
stack_alignh(small_mat) +
    align_dendro(aes(color = branch), k = 3L) +
    scale_x_reverse(expand = expansion()) +
    theme(plot.margin = margin()) +
    ggmark(mark_line(link(4:6, waiver()), link(1:2, waiver()))) +
    geom_boxplot(aes(.names, value, fill = .names)) +
...
ggheatmap(small_mat) +
    ...
    anno_right() +
    align_kmeans(3L) +
    ggmark(mark_tetragon(link(I(4:6)), link(I(1:2)))) +
    geom_boxplot(aes(.names, value, fill = .names)) +
    theme(plot.margin = margin(l = 0.1, t = 0.1, unit = "npc"))
```

You can further customize link lines and quadrilateral using `element` argument:
...
ggheatmap(small_mat) +
    ...
    align_kmeans(3L) +
    ggmark(
        mark_tetragon(link(4:6), link(1:2), 
