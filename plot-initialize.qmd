...
(`.x`/`.y`, `.names`, `.index`, and `.panel`) will be added to the data frame
if applicable. Note, `NROW(data)` must match the `number of observations` along
the axis used for alignment.
```{r}
...
powerful techniques to prepare data for your plots.
...
`quad_layout()`—most of the operations discussed in @sec-heatmap_layout can also
