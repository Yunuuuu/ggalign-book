...
accepts two arguments:
  ...
  - `link2`: Used to link observations in the right or bottom, you can use
  `waiver()` to inherit from `link1` (the same for `link1` argument, you can use
  `waiver()` to inherit from `link2` argument). This should match `link1`, as
  all plots in `stack_discrete()` should maintain the same ordering index.
  However, this becomes useful in `stack_cross()`, where different orderings are
  involved, and you must specify the link explicitly.
