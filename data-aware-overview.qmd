# Overview {#sec-data-aware-overview}
...
The data-aware composition follows a systematic process with five main steps:

| Step                         | Purpose                      | Core Functions                                       | Description                                      |
| ---------------------------- | ---------------------------- | ---------------------------------------------------- | ------------------------------------------------ |
| **1. Data Preparation**      | Standardize input formats    | `fortify_matrix()`, `fortify_data_frame()`           | Convert diverse data types to consistent formats |
| **2. Layout Initialization** | Create layout container      | `stack_layout()`, `quad_layout()`, `circle_layout()` | Define spatial arrangement structure             |
| **3. Data Alignment**        | Order and group observations | `align_order()`, `align_group()`, `align_dendro()`   | Control observation ordering and grouping        |
| **4. Plot Integration**      | Add coordinated plots        | `ggalign()`, `ggfree()`, `ggcross()`                 | Integrate plots with alignment constraints       |
| **5. Scheme Application**    | Control plot behavior        | `scheme_data()`, `scheme_theme()`, `scheme_align()`  | Fine-tune plot appearance and behavior           |

Note that during the actual application of this toolkit, some of these 5 steps may not be necessary or clearly displayed, but this workflow covers the entire general calculation process quite well, making it easier to understand how to transform input data into a result visualization.
