...
of `quad_layout()`: `quad_continuous()` sets both `xlim` and `ylim` arguments,
while `quad_discrete()` does not set `xlim` or `ylim`. For simplicity,
throughout this book, I will refer to all of the variants as `quad_layout()`.
