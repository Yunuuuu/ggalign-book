...
There are following primary functions for adding plots: 
  ...
  observation. In addition, it can reorder and group the observations based on
...
will be added to draw the dendrogram with edge data. Note that this layer is
always added to the first.
```{r}
ggheatmap(small_mat) +
    anno_top() +
    align_dendro()
...
Now that you're familiar with the general principles of adding plots and
aligning observations, it's time to explore the two layout systems provided by
the package: the circle layout and the quad layout. These layouts offer distinct
structural approaches to organizing your plots, and each is suited to different
visualization goals. Let’s dive into how they work and how to make the most of
them.
